...
class MinimaxTree:
    ...
    def dfs_explore(self, node: Node, depth_remain, start_time):
        self.nodes_search += 1
        if time.time() - start_time > THINKTIME or depth_remain == 0:
            ...
            if node.expanded == False:
                ...
                for action in new_actions:
                    ...
                    new_state = self.game_rule.generateSuccessor(
                        next_state, action, node.agent_id
                    )
                    new_node = Node(1 - node.agent_id, new_state, action, node)
                    new_node.score_difference = self.GetScore(node.state)
                    node.children.append(new_node)
                node.children.sort(key=lambda x: x.score_difference, reverse=True)
...
class myAgent:
    ...
    def SelectAction(self, actions, rootstate):
        ...
        self.minimax_tree.nodes_search = 0
        root = Node(self.id, rootstate, None, None)
        ...
        del root
        print(self.minimax_tree.nodes_search)
        return best_action
