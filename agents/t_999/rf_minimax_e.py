...
class MinimaxTree:
        ...
        if depth_remain == 0:
            ...
            for child in node.children:
                ...
                if time.time() - start_time > THINKTIME:
                    ...
                    score = self.GetScore(node.state)
    ...
    def FindNode(self, rootstate: AzulState):
        if self.root == None:
            self.root = Node(self.id, rootstate, None)
            return
        tmp = None
        for c in self.root.children:
            if c.state == rootstate:
                tmp = c
            else:
                self.delete(c)
        if tmp == None:
    ...
    def ToNode(self, destination):
        tmp = None
        for c in self.root.children:
            if id(destination) == id(c):
                tmp = c
            else:
                self.delete(c)
        self.root = tmp
...
class myAgent():
    ...
    def BestNode(self, root:Node) -> Node:
        bestNode = None
        score = -1000000
    ...
    def SelectAction(self, actions, rootstate):
        ...
        self.minimax_tree.FindNode(rootstate)
        assert self.minimax_tree != None
        ...
        best_node = self.BestNode(self.minimax_tree.root)
        best_action = best_node.action
        self.minimax_tree.ToNode(best_node)
        return best_action
