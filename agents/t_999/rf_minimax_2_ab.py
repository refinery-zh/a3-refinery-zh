...
class myAgent:
    ...
    def dfs_explore(self, actions, game_state:AzulState, depth, id, parents):
        if depth == 0:
            assert True
        ...
        for a in actions:
            ...
            if a == "ENDROUND":
                ...
                _, score = self.dfs_explore(new_actions, gs, depth - 1, 1 - id, result)
            scores.append(score)
            if parents == "root":
                continue
            if self.id == id:
    ...
    def SelectAction(self, actions, game_state):
        ...
        if depth < 1:
            depth = 1
        idx, _ = self.dfs_explore(actions, game_state, depth, self.id, "root")
        return actions[idx]
