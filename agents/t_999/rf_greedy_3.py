        ...
        for a in actions:
            ...
            if len(actions) < 60:
                ...
                for na in new_actions:
                    ...
                    if len(actions) < 15 and na != "ENDROUND":
                        new_new_actions = self.game_rule.getLegalActions(ngs, self.id)
                        for nna in new_new_actions:
                            nngs = deepcopy(ngs)
                            nngs = self.game_rule.generateSuccessor(nngs, nna, self.id)
                            new_score = GetScore(self.id, nngs)
                            scores_2.append(new_score)
                        score_2 = np.max(scores_2)
                        op_scores.append(new_score - score_2)
                    else:
